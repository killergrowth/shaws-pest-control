...
const locationContent = require('./location-content');

function buildLocationPage(city) {
  // Use written content from the pipeline if available
  const custom = locationContent[city.slug];

  const faqs = (custom && custom.faqs && custom.faqs.length)
    ? custom.faqs
    : [
  ...
  // Main body content: use written draft when available, else generic template
  const mainContent = (custom && custom.body)
    ? `${custom.body}
  ...
  const pageTitle  = (custom && custom.title)    ? custom.title    : `Pest Control in ${city.name}, KS | Shaw's Pest Control`;
  const pageDesc   = (custom && custom.metaDesc) ? custom.metaDesc : `Shaw's Pest Control serves ${city.name}, Kansas. 35+ years experience, licensed technicians, free estimates. Call (316) 251-9461.`;
...
<section>
  ...
  writePage(`${city.slug}/index.html`, page({
    ...
    schema: localBusinessSchema() + '\n' + breadcrumbSchema([{name:'Home',url:'/'},{name:'Service Areas',url:'/service-areas/'},{name:city.name,url:`/${city.slug}/`}]) + '\n' + faqSchema(faqs) + '\n' + articleSchema({ title: pageTitle, description: pageDesc, url: `https://${client.domain}/${city.slug}/`, datePublished: '2024-01-15' }),
    body,
  }));
  console.log(`  Built: /${city.slug}/ ${custom ? '✅ custom content' : '(generic)'}`);
}
