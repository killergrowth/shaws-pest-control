'use strict';
const { writePage, fill, localBusinessSchema, serviceSchema, breadcrumbSchema, faqSchema, articleSchema, lastUpdatedBar, buildDate, reviewCards, faqBlock, page, client, services, cities } = require('./shared');
...
<section class="section-dark" style="padding:100px 0 80px;background:linear-gradient(180deg,rgba(18,18,18,0.78) 0%,rgba(18,18,18,0.68) 100%),url('/images/hero.png') center/cover no-repeat;display:flex;align-items:center;">
  <div class="container">
    <div style="max-width:680px;">
      <div class="section-label" style="color:#fff;">Faith-Based &bull; Family-Owned &bull; Since 1987</div>
      <h1 style="color:#fff;margin-bottom:20px;">Pest Control.<br><span style="color:var(--kg-accent);">Under Control.</span></h1>
      ...
      <div style="display:flex;gap:32px;margin-top:40px;flex-wrap:wrap;">
        <div style="color:rgba(255,255,255,0.8);font-size:0.9rem;"><strong style="color:var(--kg-accent);font-size:1.5rem;display:block;">35+</strong>Years Experience</div>
        <div style="color:rgba(255,255,255,0.8);font-size:0.9rem;"><strong style="color:var(--kg-accent);font-size:1.5rem;display:block;">3</strong>State Certifications</div>
        <div style="color:rgba(255,255,255,0.8);font-size:0.9rem;"><strong style="color:var(--kg-accent);font-size:1.5rem;display:block;">1</strong>Master Tech Cert</div>
      </div>
    </div>
  </div>
</section>
...
<section>
  ...
  writePage('index.html', page({
    ...
    schema: localBusinessSchema() + '\n' + faqSchema(faqs) + '\n' + articleSchema({ title: "Pest Control in Butler & Sedgwick County, KS | Shaw's Pest Control", description: "Locally owned, faith-based pest control serving Butler and Sedgwick County since 1987.", url: `https://${client.domain}/`, datePublished: '2024-01-15' }),
    body,
  }));
...
function buildAbout() {
  const faqs = [
    { q: "How long has Shaw's Pest Control been in business?", a: "Shaw's Pest Control has been serving Butler and Sedgwick County since 1987 - over 35 years of trusted pest control service." },
...
<section class="section-dark" style="padding:80px 0;background:linear-gradient(145deg,#1a1a1a 0%,#2d2d2d 100%);">
  <div class="container">
    ...
    <p style="color:rgba(255,255,255,0.85);font-size:1.1rem;max-width:600px;margin-top:12px;">Reach out with a question or request a quote. Or call us directly at <a href="tel:3162519461" style="color:var(--kg-accent);font-weight:700;">(316) 251-9461</a>.</p>
  </div>
</section>
<section>
  <div class="container">
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:60px;align-items:start;">
      ...
      <div>
        <div style="background:#fff;border-radius:10px;box-shadow:0 8px 40px rgba(0,0,0,0.18);padding:36px 32px;">
          <h2 style="font-size:20px;font-weight:800;color:#1a2a24;margin-bottom:6px;">Request Your Free Estimate</h2>
          <p style="font-size:13px;color:#7E7C76;margin-bottom:24px;">We'll reach out quickly &mdash; usually within the hour.</p>
          <form action="/submit" method="POST" id="contact-form" style="display:flex;flex-direction:column;gap:14px;">
            <input type="hidden" name="page" value="contact">
            <input type="text" id="name" name="name" required placeholder="Your Name" autocomplete="name"
              style="width:100%;padding:13px 16px;border:1.5px solid #E2DDD8;border-radius:6px;font-family:inherit;font-size:15px;color:#1a2a24;background:#fafafa;transition:border-color .2s;outline:none;">
            ...
            <textarea id="message" name="message" rows="4" placeholder="Tell us about your pest problem..."
              style="width:100%;padding:13px 16px;border:1.5px solid #E2DDD8;border-radius:6px;font-family:inherit;font-size:15px;color:#1a2a24;background:#fafafa;transition:border-color .2s;outline:none;resize:vertical;min-height:90px;"></textarea>
            ...
            <button type="submit" id="form-submit-btn"
              style="background:var(--kg-primary);color:#fff;border:none;border-radius:100px;font-family:inherit;font-size:16px;font-weight:700;padding:16px 24px;cursor:pointer;transition:background .2s;width:100%;display:flex;align-items:center;justify-content:center;gap:8px;">
              &#128203; Send My Request
            </button>
            <p style="font-size:12px;color:#9E9A95;text-align:center;margin:0;">&#128274; Your info is private. We'll never spam you.</p>
            <div id="form-error" style="display:none;padding:14px 16px;background:#f8d7da;border-radius:8px;color:#721c24;font-size:14px;">Something went wrong. Please call <a href="tel:3162519461" style="color:#721c24;font-weight:700;">(316) 251-9461</a>.</div>
          </form>
          <div id="form-success" style="display:none;text-align:center;padding:20px 0;">
            <div style="font-size:48px;color:var(--kg-primary);margin-bottom:12px;">&#10003;</div>
            <h3 style="font-size:22px;font-weight:800;color:#1a2a24;margin-bottom:8px;">Got it &mdash; thank you!</h3>
            <p style="color:#7E7C76;font-size:15px;">We'll be in touch shortly. Or call us now at <a href="tel:3162519461" style="color:var(--kg-primary);font-weight:700;">(316) 251-9461</a>.</p>
          </div>
        </div>
        <style>
          #contact-form input:focus, #contact-form select:focus, #contact-form textarea:focus { border-color: var(--kg-primary); }
          #form-submit-btn:hover { background: var(--kg-primary-dark); }
        </style>
        <script>
        document.getElementById('contact-form').addEventListener('submit', async function(e) {
          ...
          try {
            ...
            btn.textContent = '\u{1F4CB} Send My Request'; btn.disabled = false;
          }
        });
        <\/script>
      </div>
      ...
      <div>
        ...
        <div class="kg-card" style="padding:28px;">
          ...
          <p style="margin-top:12px;">Monday &mdash; Friday: 8:00 AM &ndash; 5:00 PM<br>Saturday: By appointment<br>Sunday: Closed</p>
        </div>
      </div>
    </div>
  </div>
...
function buildServicePage(svc) {
  const custom = serviceContent[svc.slug];

  const faqs = (custom && custom.faqs && custom.faqs.length) ? custom.faqs : [
  ...
  // Strip raw ```json...``` schema blocks accidentally left in content HTML
  const rawHtml = (custom && custom.html) ? custom.html.replace(/```json[\s\S]*?```/g, '').replace(/<p>\s*<\/p>/g, '').trim() : null;

  const mainContent = rawHtml
    ? rawHtml
  ...
  const pageTitle = (custom && custom.title) ? custom.title : `${svc.title} in Butler & Sedgwick County, KS | Shaw's Pest Control`;
  const pageDesc  = (custom && custom.metaDesc) ? custom.metaDesc : fill(svc.description, 'Butler and Sedgwick County').substring(0, 155);
...
<section class="section-alt">
  <div class="container">
    <div class="section-title"><h2>What Do Shaw's Customers Say?</h2></div>
    <div class="kg-grid kg-grid-3">${reviewCards()}</div>
  </div>
</section>
<section>
  <div class="container">
    <div class="section-title"><div class="section-label">Service Areas</div><h2>Where Does Shaw's Provide ${svc.title}?</h2></div>
    <div class="kg-grid kg-grid-4" style="gap:16px;">
...
<section class="section-alt">
  ...
  writePage(`${svc.slug}/index.html`, page({
    ...
    schema: localBusinessSchema() + '\n' + serviceSchema(svc.name, `/${svc.slug}/`) + '\n' + breadcrumbSchema([{name:'Home',url:'/'},{name:svc.name,url:`/${svc.slug}/`}]) + '\n' + faqSchema(faqs) + '\n' + articleSchema({ title: `${svc.title} in Butler & Sedgwick County, KS | Shaw's Pest Control`, description: fill(svc.description, 'Butler and Sedgwick County').substring(0, 155), url: `https://${client.domain}/${svc.slug}/`, datePublished: '2024-01-15' }),
    body,
  }));
